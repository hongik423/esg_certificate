...
예를 들어 "BM ZEN 사업분석은 어떻게 진행되나요?", "일터혁신 상생컨설팅이 정말 무료인가요?", "정책자금으로 10억원 확보하는 방법은?" 이런 질문들 언제든 환영해요.`,
  ...
  const generateFallbackResponse = (userMessage: string): string => {
    ...
    // 키워드별 맞춤형 응답
    if (lowerMessage.includes('ai') || lowerMessage.includes('생산성') || lowerMessage.includes('자동화')) {
      return `AI 생산성 향상에 대해 문의해주셔서 감사합니다! 🤖
...
🏆 **성공사례:**
(주)스마트팩토리솔루션 - 제안서 작성 69% 단축, 연간 5억 8천만원 효과
...
📞 **상담:** 010-9251-9743`;
    ...
    if (lowerMessage.includes('정책자금') || lowerMessage.includes('지원금') || lowerMessage.includes('보조금')) {
      return `정책자금에 대해 문의해주셔서 감사합니다! 💰
...
🏆 **성공사례:** H관광개발 - 5년간 100억원 확보, 매출 650% 성장

📞 **상담:** 010-9251-9743`;
    }
    
    if (lowerMessage.includes('창업') || lowerMessage.includes('스타트업')) {
      return `기술창업에 대해 문의해주셔서 감사합니다! 🚀

**4단계 성장 로드맵:**
1️⃣ 특허출원+예비벤처 (1.7억원)
2️⃣ 벤처확인+기술개발 (24.7억원)  
3️⃣ TIPS선정+VC투자 (57억원)
4️⃣ 상장준비+글로벌확대

🏆 **성공사례:** ABC기업 - 총 87억원 확보, ROI 2,174%

📞 **상담:** 010-9251-9743`;
    }
    
    if (lowerMessage.includes('세금') || lowerMessage.includes('계산기')) {
      return `세금계산기에 대해 문의해주셔서 감사합니다! 🧮
